...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.onload = function() {
    const textDescriptions = document.querySelectorAll('.text__desc');

    const readBtns = document.querySelectorAll('.readMoreBtn');
    
    textDescriptions.forEach((textDescription, index) => {
      const copiedTxt = textDescription.textContent;
      
    
      if (textDescription.textContent.length > 20) {
        console.log('hey less than 20');
        textDescription.textContent = `${textDescription.textContent.slice(0, 400)}....`;
        readBtns[index].innerHTML = `<span>Read More</span><i class="bi bi-arrow-right"></i>`;
      }
